...
<?php
require "login.php";
if (!isset($_SESSION['txtus'])) // Nếu session == null thì trả về trang login
{
    header("Location: account.php");
    exit; // Đảm bảo kết thúc việc thực hiện mã PHP sau khi chuyển hướng
}

// Kiểm tra xem giỏ hàng có sản phẩm hay không
if (!isset($_SESSION['cart']) || count($_SESSION['cart']) === 0) {
    header("Location: baogiohangtrong.php");
    exit; // Kết thúc việc thực hiện mã PHP sau khi chuyển hướng
}
?>
...
function laygiatheoiddichvu(str) {
  ...
  if (str.length == 0) { 
		...
		div.innerHTML = thannhtien.value +".000 VNĐ";
